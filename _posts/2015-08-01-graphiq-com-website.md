...
desc: Create a versatile company about site with minimal efforts and provide a way for the company to configure the content with ease.
...
When we launched [Graphiq.com][1]{:target="\_blank"}, we wanted to quickly build a nice-looking website and move the content out of our codebase into marketing’s hands.

I used a paid framework to save time, made content blocks into templates, and defined a <span class="small-caps">JSON</span> file format, so marketing department can have full control of the site’s content  without us changing a line of code.

[![Graphiq.com]({{ site.baseurl }}/assets/img/home-graphiq.jpg)](//graphiq.com){:target="\_blank"}

### Background

August 2015, we rebranded [FindTheBest.com][2]{:target="\_blank"} to be [Graphiq.com][3]{:target="\_blank"}. An important part of that was to make a new company website to host all the marketing pages—about us, product introductions, <span class="small-caps">Q&A</span>, etc.

Previously, we put all the raw <span class="small-caps">HTML</span> for those pages directly in our codebase. So for every single change the marketing department wanted to make, they had to ask an engineer and wait for the deploy. With the new website, I wanted to find a way to give marketing department the power to directly make changes without going through us.
...
With these templates, we can empower the marketing team and give them the control over the content of the site.
...
We also implemented a test file that gets used with special <span class="small-caps">URL</span> parameter so marketing can test their content before pushing the changes to the public.

### Extension

After the project, I thought the <span class="small-caps">JSON</span> file is such a simple yet powerful way of constructing a page. That inspired me to create [Premade.io][5]{:target="\_blank"} as a side project to help more people make webpages by organizing pre-defined content blocks and filling their own content.
